...
export async function getClienteByIdForTenant(
  ...
  const r = await pool.query<Record<string, unknown>>(
    `SELECT id, tenant_id, documento, nome, email, whatsapp_opt_in, created_at, updated_at
     FROM portal.cliente
...
export async function listClientesByTenantPage(
  tenantId: string,
  options: { limit: number; cursor?: ClienteKeysetCursor | null },
  pool: Pool = getPool()
...
export async function insertCliente(
  ...
  const email =
    input.email?.trim() || `sem-email+${input.documento.slice(-6)}@cobranca.local`;

  const r = await pool.query<Record<string, unknown>>(
    `INSERT INTO portal.cliente (
       tenant_id, documento, tipo_documento, nome, email, whatsapp_opt_in, opt_in_whatsapp
     )
     VALUES ($1, $2, $3, $4, $5, $6, $6)
     RETURNING id, tenant_id, documento, nome, email, whatsapp_opt_in, created_at, updated_at`,
    [tenantId, input.documento, tipoDocumento, input.nome, email, input.whatsappOptIn]
  );
  const row = r.rows[0];
  if (!row) {
...
export async function updateClienteForTenant(
  ...
  const r = await pool.query<Record<string, unknown>>(
    `UPDATE portal.cliente
     SET nome = $3, email = $4, whatsapp_opt_in = $5, updated_at = now()
     WHERE id = $1::uuid AND tenant_id = $2
     RETURNING id, tenant_id, documento, nome, email, whatsapp_opt_in, created_at, updated_at`,
    [id, tenantId, nome, email, whatsappOptIn]
  );
  const row = r.rows[0];
  return row ? mapRow(row) : null;
}
